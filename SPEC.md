...
> ⚠️ 이 폴더 루트의 `PRD.md` / `AGENTS.md`는 **"모두의 계획서"라는 다른 프로젝트**의 것이다. 본 스펙은 별도 기획서인 캠퍼스 탐방 PWA PRD를 기준으로 한다. 저장소를 분리하거나 하위 디렉터리로 격리할 것.
...
**결정 (D1=B):** PRD §9.4의 7일 규칙은 **그대로 유지**한다. 다만 최근 7일 결과가 **0건일 때만** 그 아래에 "지난 공지" 섹션으로 최신 3건을 `--text-muted` 회색으로 덧붙인다. 규칙을 바꾸는 게 아니라 **빈 상태의 대체 표현**이므로 기획서와 어긋나지 않는다.
...
| Supabase 프로젝트 | ❌ 미생성 | S1 |

**전부 임시본으로 먼저 개발하고 파일만 교체한다** (PRD §8). 캐릭터는 동일 파일명·동일 캔버스의 플레이스홀더로 시작한다.
...
**빈 상태 폴백 (D1 결정):** 최근 7일 결과가 0건이면 `최근 7일 안에 올라온 공지가 없어요`를 표시하고, 그 아래 `지난 공지` 섹션으로 최신 3건을 `--text-muted`로 덧붙인다. 7일 결과가 1건 이상이면 이 섹션은 렌더링하지 않는다.
...
11. 7일 결과 0건일 때 빈 상태 문구 + `지난 공지` 3건이 표시된다.
...
| 발표 당일 공지 0건 | 세 출처 모두 7일 내 1건뿐 (실측) | 빈 상태 + `지난 공지` 3건 폴백 (D1=B) |
...
| ID | 결정 | 근거 |
|---|---|---|
| D1 | 7일 규칙 유지 + 0건일 때 `지난 공지` 3건 폴백 | 실측상 장소당 1건뿐이라 데모가 빌 위험. 규칙을 바꾸지 않고 빈 상태 표현만 보강 |
