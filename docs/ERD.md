...
| Apify MCP | 사용자 요청으로 전환, 현재 세션 도구 미연결 | OAuth 연결 후 세 출처 Actor 실행과 결과 검증 |
| Apify→Supabase 수동 반영 | 미구현 | 연결 후 운영자 요청 한 번으로 세 Actor 실행과 저장 결과 검증 |

Apify MCP를 아직 연결하지 못했으므로 공지 크롤링 완료로 판단하지 않는다. 임시 데이터로 UI를 개발할 수는 있지만, 최종 크롤링 증거를 대신할 수는 없다.
